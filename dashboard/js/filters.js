...
Swift.Filter = (function () {
  ...
  function refreshDynamicOptions() {
    ...
    // Build scatter lookup: centro_nv2 → regiao
    const scatterPts = DATA.qualitativo?.scatter_transacoes || [];
    const qrm = {};
    scatterPts.forEach(p => { qrm[p.centro_nv2] = p.regiao; });
    ...
    const regioes = [...new Set(lojas.map(l => qrm[l.nome]).filter(Boolean))].sort();
    if (f.regiao.length) lojas = lojas.filter(l => f.regiao.includes(qrm[l.nome]));
